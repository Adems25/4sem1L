...
class RBTree
{
	...
	Node* get_Node(int k, Node* cur)
	{
		...
		if (cur->key == k)
			return cur;
		if (cur->key < k)
			return get_Node(k, cur->right);
		if (cur->key > k)
			return get_Node(k, cur->left);
	}
	...
	Node* insert_ser(int IncludeKey, char IncludeValue, Node* Parent)
	{
		...
		if (IncludeKey > Parent->key)
		{
			if (!Parent->right)
			{
				...
				return insert_ser(IncludeKey, IncludeValue, Parent->right);
		}
		else
		{
			if (!Parent->left)
			{
				...
				return insert_ser(IncludeKey, IncludeValue, Parent->left);
		}
	}
public:
	RBTree() { root = nullptr; }
	void insert(int Key, char Value)
	{
		...
		{
			Node* newNode;
			try { newNode = insert_ser(Key, Value, root); }
	...
	void remove(int Key)
	{
		if (!root)
			throw runtime_error("Tree is empty\n");
		Node* delNode = get_Node(Key, root);
		if (!delNode)
			throw runtime_error("Node isn't exist\n");
		if (delNode->left && delNode->right)
		{
			Node* temp = delNode;
			delNode = get_Min(delNode->right);
